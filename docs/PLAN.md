...
| V2.3 | ✅ shipped | Unified `+ New` entry — sidebar dialog with 4 cards, pre-attaches voice + knowledge to the destination |
| **V2.4** | **next** | **Run timeline drawer + step metrics (UX Phase 6)** |
...
- [x] **Phase 5 — Unified `+ New` entry** (V2.3): [src/components/compose/new-entry-dialog.tsx](../src/components/compose/new-entry-dialog.tsx) opens from a primary `+ New` button at the top of the sidebar. Asks "What are you making?" with four cards — Single asset / Campaign / Long-form / Brainstorm — each visually distinguished by an accent colour. Reuses `<ComposeContextBar>` so the voice + knowledge picked in the sheet pre-attaches to the destination: forms (Copywriter / Campaign) get the values via URL params (`?voiceId=…&locale=…&sourceIds=a,b`) and read them on mount, while Long-form / Brainstorm call `createDocument` / `createChatThread` directly with the picked context and redirect to the new resource. URL params are validated against the workspace's available IDs so stale links can't pre-pick a missing voice.
