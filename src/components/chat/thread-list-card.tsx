...
export function ThreadListCard({ thread }: Props) {
  return (
    <Link
      href={`/chat/${thread.id}`}
      className={cn(
        "group flex flex-col gap-2 rounded-xl border border-[--color-border] bg-[--color-card] p-5",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {thread.pinned && <Pin className="h-3.5 w-3.5 text-[--color-primary]" />}
      </div>
      ...
      {thread.lastMessagePreview && (
        <p
          className="text-sm text-[--color-muted-foreground] line-clamp-3 text-pretty"
